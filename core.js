...
class TikTokAutomation {
    ...
    async login() {
        try {
            ...
            // Email input
            const emailInput = await this.page.waitForSelector('input[placeholder="Email or username"]', { timeout: 15000 });
            ...
            await this._randomDelay(4000, 7000);
            
            // Check success
            const currentUrl = this.page.url();
            if (currentUrl.includes('feed') || currentUrl.includes('foryou') || currentUrl.includes('user')) {
                logger.info(`Login successful: ${this.account.email}`);
                return true;
            }
            
            // Check for captcha
            const hasCaptcha = await this.page.isVisible('iframe[title*="captcha"]');
            if (hasCaptcha) {
                logger.warn(`Captcha detected for ${this.account.email}`);
                return false;
            }
    ...
    async reportVideo(videoUrl, reason = 'violent_acts') {
        try {
            ...
            const moreBtn = await this.page.waitForSelector(
                'button[aria-label="More actions"], div[data-e2e="more-actions"]',
                { timeout: 10000 }
            ).catch(() => null);
            ...
            // Report button
            const reportBtn = await this.page.waitForSelector(
                'button:has-text("Report"), div:has-text("Report"), span:has-text("Report")',
                { timeout: 5000 }
            ).catch(() => null);
            ...
            // Select reason
            const reasonMap = {
                violent_acts: ['Violent acts', 'Violence'],
                spam: ['Spam'],
            ...
            for (const text of reasonTexts) {
                const reasonBtn = await this.page.waitForSelector(
                    `div:has-text("${text}"), span:has-text("${text}")`,
                    { timeout: 3000 }
                ).catch(() => null);
            ...
            const submitBtn = await this.page.waitForSelector(
                'button:has-text("Submit"), button:has-text("Report")',
                { timeout: 5000 }
            );
